...
import {
  hasSpeedBonus,
  resolveSpeedBonusTiers,
  speedBonusForRank,
} from '../lib/showdownSpeedBonus.js';
import {
...
async function setClassActiveShowdownPointer(classCode, sessionId) {
  ...
  try {
    ...
    // non-fatal: pointer is a convenience for auto-discovery
  }
}
...
function roundSeconds(session, roundId) {
...
function computePoints(session, round, isCorrect, finishChoice) {
...
/**
 * Speed bonus by submission rank among correct answers (rank 0 = fastest).
 * Uses matrix `speedBonusTiers`: e.g. [5, 4, 3] → +5, +4, +3 for top 3.
 */
function rankedSpeedBonus(rank, tiers) {
